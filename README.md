...
在「批次補登」頁,機台選「總收入(舊資料,無機台明細)」「洗衣機總收入(舊資料,無機台明細)」或「烘衣機總收入(舊資料,無機台明細)」時,會多一個「總金額(舊資料)」欄位,可以直接填當天總收入,不用硬拆成幾個 10 元、幾張 100 元。這個總額存在資料庫的 `manual_total` 欄位,分析頁會優先使用這個數字當作那筆紀錄的總計。
...
批次補登現在不管選哪台機台,都可以只填最右邊的「總金額」欄位,不強制要求面額明細(投幣機現鈔也一樣)。

## 投幣機現鈔:總金額 + 1000/500 元張數

「投幣機現鈔」不算進店裡營收(看分析頁的統計和趨勢圖都會自動排除它,但另外有一張自己的趨勢圖)。`entry.html` 裡這台機器除了總金額,還多兩個欄位可以填 1000 元、500 元各幾張。2026-06-01 之後的日期,送出時會自動把這兩個數字寫成「1000元 X張、500元 Y張」加進備注欄。
